var logotree = document.querySelector('.logothree');
logotree.addEventListener('click', ()=> { window.location.replace("/index.html");});
...
class Constraints {
    static generate(args, resultingChecks) {
        ...
        if (resultingChecks) {
            ...
            result.moves = result.moves.filter((location) => !resultingChecks({ piece, location, capture: false, moveIndex })
                .length);
...
class Piece {
    ...
    dir(stepsRow, stepsColumn, positions) {
        ...
        const col = Utils.colToInt(positions[this.data.id].col) +
...
class Game {
    ...
    activate(location) {
        ...
        if (tilePiece && !this.active && tilePiece.data.player !== this.turn) {
            ...
            this.activePieceOptions = [];
            const capturePiece = (validatedPosition === null || validatedPosition === void 0 ? void 0 : validatedPosition.capture)
...
class Control {
    ...
    autoplay() {
        const input = this.game.turn === "WHITE"
